...
const useKaitoriAndroid = () => {
  ...
  const url = "/api/kaitori/android-prices";
  const CACHE_KEY = "kaitori_android_data";
  const CACHE_TIMESTAMP_KEY = "kaitori_android_timestamp";
    ...
    try {
      ...
      if (cachedData && cachedTimestamp) {
        ...
        if (now - timestamp < CACHE_DURATION) {
          ...
          setAndroids(parsedData.products || []);
          setStats({
  ...
  const fetchAndroids = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        if (data.status === "success") {
          ...
          throw new Error(data.error || "Failed to fetch Android data");
        }
      } catch (err) {
        setError(err.message);
        console.error("Error fetching Android data:", err);
      } finally {
        setLoading(false);
      }
    },
  ...
  const getChangedProducts = useCallback(() => {
    return androids.filter((phone) => phone.changed === true);
  }, [androids]);

  // Get products by model/name
  const getProductsByModel = useCallback(
    (modelName) => {
      return androids.filter((phone) =>
...
export default useKaitoriAndroid;
